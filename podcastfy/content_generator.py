...
class LongFormContentGenerator:
    ...
    def enhance_prompt_params(self, prompt_params: Dict,
        ...
        if part_idx == 0:
            ...
            ALWAYS START THE CONVERSATION GREETING THE AUDIENCE: 
            ALWAYS START BY SETTING THE CONTEXT BRIEFLY.
    ...
    def generate_long_form(
        ...
        # ✅ Estimate per-part word target
        words_per_part = (max_words // num_parts) if max_words else None
        print(f"Generating {num_parts} parts with ~{words_per_part or '∞'} words each", flush=True)
        ...
        for i, chunk in enumerate(chunks):
            ...
            if words_per_part:
                enhanced_params["instruction"] += f"\nLimit this section to approximately {words_per_part} words."
...
class LongFormContentStrategy(ContentGenerationStrategy, ContentCleanerMixin):
    ...
    def generate(self,
             ...
             input_texts: str,
             prompt_params: Dict[str, Any],
             max_words: Optional[int] = None  # ✅ add this
) -> str:
        generator = LongFormContentGenerator(chain, self.llm, self.config_conversation)

        # ✅ Inject a max word count instruction into the prompt
        if max_words:
            prompt_params["user_instructions"] = (
                f"Please limit the total generated output to approximately {max_words} words.\n\n"
                + prompt_params.get("user_instructions", "")
            )

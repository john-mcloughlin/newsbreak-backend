...
class TextToSpeech:
    ...
    def check_quota(self) -> bool:
        try:
            self.convert_to_speech("Test quota check.", "/dev/null")
            return True
        except Exception as e:
            if "429" in str(e) or "quota" in str(e).lower():
                return False
            raise  # raise if it's some other error
    ...
    def convert_to_speech(self, text: str, output_file: str) -> None:
        ...
        """
        print("🎙️ [TTS] Starting text-to-speech conversion...", flush=True)
        cleaned_text = text

        try:
            if "multi" in self.provider.model.lower():
                print("🧪 [TTS] Using multi-speaker model...", flush=True)
                provider_config = self._get_provider_config()
                voice = provider_config.get("default_voices", {}).get("question")
                voice2 = provider_config.get("default_voices", {}).get("answer")
                model = provider_config.get("model")
                print("🔊 [TTS] Requesting multi-speaker audio from provider...", flush=True)
                ...
                try:
                    ...
                    print(f"🎧 [TTS] Received {len(audio_data_list)} chunks. Merging now...", flush=True)
                    combined = AudioSegment.empty()

                    for i, chunk in enumerate(audio_data_list):
                        segment = AudioSegment.from_file(io.BytesIO(chunk))
                        print(f"🔉 [TTS] Merged chunk {i + 1}/{len(audio_data_list)} - duration: {len(segment)}ms", flush=True)
                        combined += segment
                    ...
                    print(f"✅ [TTS] Audio saved to: {output_file}", flush=True)

                except Exception as e:
                    logger.error(f"Error during audio processing: {str(e)}")
                    raise
            else:
                print("🗣️ [TTS] Using single-speaker model (Edge/OpenAI)...", flush=True)
                with tempfile.TemporaryDirectory(dir=self.temp_audio_dir) as temp_dir:
                    ...
                    print(f"🔗 [TTS] Merging {len(audio_segments)} audio segments...", flush=True)
    ...
    def _generate_audio_segments(self, text: str, temp_dir: str) -> List[str]:
        ...
        for idx, (question, answer) in enumerate(qa_pairs, 1):
            for speaker_type, content in [("question", question), ("answer", answer)]:
                ...
                print(f"🎤 [TTS] Generating {speaker_type} {idx} (length: {len(content)} characters)", flush=True)
